...
const disasterData = [
  {
    id: 1,
    title: 'Earthquake',
    description: 'Seismic activity causing ground shaking and potential structural damage',
    color: 'bg-red-500',
    guidelines: [
...
const DisasterCard = ({ disaster, onClick }) => (
  ...
  >
    <h3 className="text-2xl font-bold text-white text-center px-4">{disaster.title}</h3>
  </Card>
);
            ...
            <ul className="space-y-3">
              {disaster.guidelines.map((guideline, index) => (
                <li key={index} className="flex items-start">
                  <span className={`${disaster.color} text-white w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 mr-3`}>
                    {index + 1}
                  </span>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold text-center mb-12">Natural Disasters Information</h1>
